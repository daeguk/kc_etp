...
export default {
  ...
  mounted: function() {
    ...
    $('.selectAll').on('click', function () {
        if ($(this).is( ":checked" )) {
            kospi_grid.rows().select();        
        } else {
            kospi_grid.rows().deselect(); 
        }
    });
  },
  methods: {

    getKospiList: function() {

//      this.results = this.$store.state.etfmast;
      if (!$.fn.dataTable.isDataTable( '#kospi_grid' ) ) {
        kospi_grid = $('#kospi_grid').DataTable( {
          "processing": true,
          "serverSide": false,
          "search": true,
          scrollY:        '50vh',
    ...
    selectData: function() {
      console.log("data=" + kospi_grid.rows( { selected: true } ).count());
      var data = kospi_grid.rows( { selected: true } ).data();
