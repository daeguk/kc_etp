...
-->
 <v-card flat>
  <table id="kosdaq_grid" class="tbl_type" style="width:100%;">
    <colgroup>
      <col width="15%">
      <col width="30%">
      <col width="55%">
...
export default {
  ...
  created: function() { 
	...
	vm.$EventBus.$on('fn_kosdaqFilterData', data => {
		vm.search = data;
		vm.filterData();
	});		  
  },
  beforeDestroy() {
      var vm = this;
      vm.$EventBus.$off('fn_kosdaqFilterData');
  },
  mounted: function() {
    var vm = this;
    ...
    $('.selectAll').on('click', function () {
        if ($(this).is( ":checked" )) {
            kosdaq_grid.rows().select();        
        } else {
            kosdaq_grid.rows().deselect(); 
        }
    });
  },
  methods: {
    getKosdaqList: function() {
//      this.results = this.$store.state.etfmast;
      if (!$.fn.dataTable.isDataTable( '#kosdaq_grid' ) ) {
        kosdaq_grid = $('#kosdaq_grid').DataTable( {
          "processing": true,
          "serverSide": false,
          "search": true,
          scrollY:        '50vh',
    ...
    selectData: function() {
      console.log("data=" + kosdaq_grid.rows( { selected: true } ).count());
      var data = kosdaq_grid.rows( { selected: true } ).data();
