...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async newAccount() {
        var vm = this;
        if(this.editedItem.type_cd.length == 0 || this.editedItem.type_cd == "0000") {
          vm.status = 1;
        ...
        } else if(this.editedItem.inst_cd.length == 0 || this.editedItem.inst_cd == "00000") {
          vm.status = 1;
        ...
        } else if(this.editedItem.password.length == 0 || this.editedItem.password.length == 0 || this.editedItem.password != this.editedItem.password) {
          vm.status = 1;
          ...
          if(this.editedItem.email.includes(this.editedItem.domain_url)) {
            axios.post(Config.base_url + '/user/member/usernewaccount', vm.editedItem).then(async function(response) {
              console.log(response);
              if(response.data.success == false) {
                vm.status = 1;
                if(await vm.$root.confirmt.open('확인', response.data.message, {}, 1)) {
