...
def set_data(mystr):
    ...
    month_idx['prev'] = month_idx['backtest'].shift(1)
    month_idx['rtn'] = month_idx['backtest'].pct_change()
    month_idx['bm_prev'] = month_idx['benchmark'].shift(1)
    month_idx['bm_rtn'] = month_idx['benchmark'].pct_change()
    month_idx['kospi_rtn'] = month_idx['kospi'].pct_change()

    month_idx = month_idx['2001-01':'2019-04']
...
def analyze_data(df):
    ...
    # bonus 1year momentum (bonus) 
    momentum = np.prod(1+df['rtn'].tail(12).values) - 1
    result['momentum_1y'] = momentum
...
if __name__ == '__main__':
    ...
    #json 파일을 읽어서, 스트링으로 변환. 나중에 해당 스트링을 argv로 받을 예정
    #배포시 아래로 변경
    in_str = open("mytest.json").read()
    #in_str = sys.argv[1]
