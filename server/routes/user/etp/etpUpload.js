  ...
  upload(req, res, function(err) {
    ...
    try {
      ...
      var dataLists = xlsx.utils.sheet_to_json(workbook.Sheets[sheet_name_list[0]], {
        header: ["col01", "col02", "col03", "col04", "col05"],
      ...
      if(dataLists.length <= 1) {
        ...
        /* 비어 있는 열이 존재하는지 체크 */
        if((typeof dataLists[0].col01 == "undefined" || dataLists[0].col01 == "") || (typeof dataLists[0].col02 == "undefined" || dataLists[0].col02 == "") || (typeof dataLists[0].col03 == "undefined" || dataLists[0].col03 == "") || (typeof dataLists[0].col04 == "undefined" || dataLists[0].col04 == "") || (typeof dataLists[0].col05 == "undefined" || dataLists[0].col05 == "")) {
                  ...
                  conn.query(stmt, function(err, rows) {
                    ...
                    if(rows && rows.length > 0) {
                      ...
                      rows.forEach(function(item, index, array) {
                        ...
                        data.fmt_F16499 = Math.floor(Number(data.F16499) * 100) / 100; /* 1CU단위증권수 */
                        data.F33861 = item.F33861; /* ETF시장구분 */
                        data.F34840 = item.F34840; /* 액면금액설정현금액 */
                        data.fmt_F34840 = Math.floor(Number(data.F34840) * 100) / 100; /* 액면금액설정현금액 */
                        data.F33904 = (typeof item.F33904 == "undefined" || item.F33904 == null ? "" : item.F33904); /* 선물 옵션 계약승수*/
                        ...
                        if(filterData && filterData.length == 1) {
                          ...
                          data.fmt_F34840 = Math.floor(Number(data.F34840) * 100) / 100; /* 액면금액설정현금액 */
                          if(!(Number(data.F16499_prev) == Number(filterData[0].F16499) && Number(data.F34840_prev) == Number(item.F34840))) {
                            data.status = "modify";
                            if(data.status == "modify") {
                              if(Number(data.F16499_prev) == 0) {
                                data.F16588 = 0; /* 평가금액 */
                              } else {
                                data.F16588 = (Number(data.F16588_prev) * Number(filterData[0].F16499) / Number(data.F16499_prev)); /* 평가금액 */
                              }
                            }
                          }
                          if(!(Number(data.F34840_prev) == Number(filterData[0].F34840) && Number(data.F16499_prev) == Number(item.F16499))) {
                            data.status = "modify";
                            if(data.status == "modify") {
                              if(Number(data.F34840) == 0) {
                                ...
                                if(Number(data.F34840_prev) == 0) {
                                  ...
                                  data.F16588 = (Number(data.F16588_prev) * Number(filterData[0].F34840) / Number(data.F34840_prev)); /* 평가금액 */
                                }
                              }
                            }
                          }
                      ...
                      try {
                        ...
                        for(var i = 0; i < msg.arr_insert_list.length; i++) {
                          ...
                          if(filterData && filterData.length == 1) {
                            ...
                            data.F34840 = item.F34840; /* 액면금액설정현금액 */
                            data.fmt_F34840 = Math.floor(Number(data.F34840) * 100) / 100; /* 액면금액설정현금액 */
                            data.F33904 = (typeof filterData[0].F33904 == "undefined" || filterData[0].F33904 == null ? 0 : filterData[0].F33904); /* 선물 옵션 계약승수*/
                            data.F34743 = 0; /* ETF_PDF비중 */
                            data.fmt_F34743 = 0; /* ETF_PDF비중 */
                            data.F16004 = filterData[0].F16002; /* 해외시장종목명 */
                            data.status = "insert";
                            item.status = data.status;
                            data.F15007 = (typeof filterData[0].F15007 == "undefined" || filterData[0].F15007 == null ? 0 : filterData[0].F15007); /* kspjong_basic 기준가 (신규추가시 사용) */
                            data.F16588 = 0;
                            ...
                            if(Number(item.F34840) > 0) {
                              ...
                              if(Number(item.F34840) == 0) {
                                if(data.F33861 == '4') {
                                  data.F16588 = (Number(data.F15007) * Number(data.F16499) * Number(data.F33904));
                                }
                ...
                if(!v_arr_change_list || v_arr_change_list.length == 0) {
                  ...
                  if(v_arr_insert_check && v_arr_insert_check.length > 0) {
                    for(var i = 0; i < v_arr_insert_check.length; i++) {
                      ...
                      resultMsg.errorList.push({
                        msg: "[" + (item.row_no) + " 행] 종목명을 입력해 주세요."
                      });
                      /* 10 개 까지만 결과정보에 보관한다. */
                      if(resultMsg.errorList.length == 10) {
                        break;
                    ...
                    v_arr_insert_check = _.filter(dataLists, function(o) {
                      return (typeof o.status == "undefined" || o.status == "") && !_.some(_.map(msg.v_arr_insert_dest, w => o.F16316.includes(w.com_dtl_cd)))
                    });
                    if(v_arr_insert_check && v_arr_insert_check.length > 0) {
                      for(var i = 0; i < v_arr_insert_check.length; i++) {
                        var item = v_arr_insert_check[i];
                        resultMsg.errorList.push({
                          msg: "[" + (item.row_no) + " 행] '" + item.F16316 + "'를 찾을 수 없습니다. 입력값을 다시확인해주세요."
                        });
                        /* 10 개 까지만 결과정보에 보관한다. */
                        if(resultMsg.errorList.length == 10) {
                          break;
                      ...
                      msg.pdf_list = _.orderBy(msg.pdf_list, ["fmt_F12506", "status"], ["asc", "asc"]);
