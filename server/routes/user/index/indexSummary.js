...
var getIndexSummaryInfo = function(req, res) {
  try {
    ...
    var stmt1 = mapper.getStatement('index', 'indexSummaryLately', params, {
      language: 'sql',
      indent: '  '
    });
    var stmt2 = mapper.getStatement('index', 'indexSummaryResult', params, {
      language: 'sql',
      indent: '  '
    });
    // 대입 문자 치환
...
var getInfoOpenReqList = function(req, res) {
  try {
    ...
    if(req.session.type_cd == '9998' || req.session.type_cd == '9999') params.large_type = "FNGUIDE";
    var stmt = mapper.getStatement('index', 'indexReqList', params, {
      language: 'sql',
      indent: '  '
    });
    log.debug(stmt);
...
var getindexSubscribeList = function(req, res) {
  try {
    ...
    if(req.session.type_cd == '9998' || req.session.type_cd == '9999') params.large_type = "FNGUIDE";
    var stmt = mapper.getStatement('index', 'getindexSubscribeList', params, {
      language: 'sql',
      indent: '  '
    });
    log.debug(stmt);
...
var updateIndexOpenYn = function(req, res) {
  try {
    ...
    var stmt = mapper.getStatement('index', 'updateIndexOpenYn', params, {
      language: 'sql',
      indent: '  '
    });
    log.debug(stmt);
...
var getInfoIndexList = function(req, res) {
  ...
  if(req.session.type_cd == '9998' || req.session.type_cd == '9999') params.large_type = "FNGUIDE";
  var stmt = mapper.getStatement('index', 'getInfoIndexList', params, {
    language: 'sql',
    indent: '  '
  });
  log.debug(stmt);
...
var getIndexSummaryHist = function(req, res) {
  try {
    ...
    var stmt = mapper.getStatement('index', 'selectIndexSummaryHist', options, {
      language: 'sql',
      indent: '  '
    });
    log.debug(stmt);
...
var getIndexBaseInfo = function(req, res) {
  try {
    ...
    var stmt = mapper.getStatement('index', 'getIndexBaseInfo', options, {
      language: 'sql',
      indent: '  '
    });
...
var getIndexEtpHistoryData = function(req, res) {
  try {
    ...
    if(req.session.type_cd == '9998' || req.session.type_cd == '9999') options.large_type = "FNGUIDE";
    log.debug("options", JSON.stringify(options));
    var stmt = mapper.getStatement('index', 'getIndexEtpHistoryData', options, {
      language: 'sql',
      indent: '  '
    });
    log.debug("stmt", stmt);
...
var getIndexInEtpInfo = function(req, res) {
  try {
    ...
    if(req.session.type_cd == '9998' || req.session.type_cd == '9999') options.large_type = "FNGUIDE";
    log.debug("options", JSON.stringify(options));
    var stmt = mapper.getStatement('index', 'getIndexInEtpInfo', options, {
      language: 'sql',
      indent: '  '
    });
    log.debug(stmt);
...
var getIndexImportanceList = function(req, res) {
  try {
    ...
    if(req.session.type_cd == '9998' || req.session.type_cd == '9999') options.large_type = "FNGUIDE";
    log.debug("options", JSON.stringify(options));
    var stmt = mapper.getStatement('index', 'getIndexImportanceList', options, {
      language: 'sql',
      indent: '  '
    });
    // 대입 문자 치환
...
var getIndexAnalysisInfo = function(req, res) {
  try {
    ...
    if(req.session.type_cd == '9998' || req.session.type_cd == '9999') options.large_type = "FNGUIDE";
    log.debug("options", JSON.stringify(options));
    var stmt = mapper.getStatement('index', 'getIndexAnalysisInfo', options, {
      language: 'sql',
      indent: '  '
    });
    // 대입 연산자 치환
...
var getIndexAnalysisData = function(req, res) {
  try {
    ...
    var stmt = mapper.getStatement('index', query_id, options, {
      language: 'sql',
      indent: '  '
    });
    // 대입 연산자 치환
...
var getShareReqCnt = function(req, res) {
  try {
    ...
    // var options = {id:'admin'};
    var options = {};
    var stmt = mapper.getStatement('index', 'getShareReqCnt', options, {
      language: 'sql',
      indent: '  '
    });
    Promise.using(pool.connect(), conn => {
...
var getIndexRegStateCnt = function(req, res) {
  try {
    ...
    var stmt = mapper.getStatement('index', 'getIndexRegStateCnt', options, {
      language: 'sql',
      indent: '  '
    });
    Promise.using(pool.connect(), conn => {
