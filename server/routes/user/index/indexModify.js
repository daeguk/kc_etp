          ...
          Promise.using(pool.connect(), conn => {
            conn.beginTransaction(txerr => {
              ...
              async.waterfall([
                ...
                function(data, callback) {
                  try {
                    if(paramData.prev_method_file_id != -1 && paramData.prev_method_file_id != paramData.method_file_id) {
                      paramData.file_id = paramData.prev_method_file_id;
                  ...
                  try {
                    if(paramData.prev_method_file_id != -1 && paramData.prev_method_file_id != paramData.method_file_id) {
                      ...
                      if(paramData.prev_mothod_save_file_name) {
                        deleteFile({
                          "uploadFolder": reqParam.uploadFolder,
                          "save_file_name": paramData.prev_mothod_save_file_name
                        });
                        callback(null, paramData);
                      } else {
                        callback(null, paramData);
