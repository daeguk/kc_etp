    ...
    for(var i = 0; i < Object.keys(p_simulPortfolioObj).length; i++) {
      ...
      if(typeof JongmokImportDateList[v_portKey] == "undefined") {
        ...
        JongmokImportDateList[v_portKey] = {
          'index': -1,
          /* 전후 리밸런싱 날짜를 찾기 위해 p_arrRebalanceDate index 저장 */ 'history_start_date': '',
          /* 종목 이력 첫 날짜 */ 'IMPORT_YN': '0',
          /* 종목 편입 여부 */ 'startDate': '' /* 종목 편입 일자 */
        }
...
var fn_set_dayilyJongmok = function(
  ...
  var totalInfo = {
    ...
    tot_F15028_1_TODAY_RATE: 0 /* 종가 * 상장주식수 * 지수적용비율 총액 */ ,
    tot_F15028_2_TODAY_RATE: 0 /* 기준가 * 상장주식수 * 지수적용비율 총액 */ ,
    tot_F15028_S_TODAY_RATE: 0 /* (첫날 종가, 이후 기준가) * 상장주식수 * 지수적용비율 총액 */ ,
    tot_F15028_S: 0 /* 기준 시가총액 */ ,
    tot_F15028_C: 0 /* 비교 시가총액 */ ,
    tot_F15028_J: 0 /* 종가 기준 시가총액 */ ,
  ...
  try {
    ...
    for(var i = 0; i < Object.keys(p_simulPortfolioObj).length; i++) {
      ...
      */
      ...
      if(JongmokImportDateList[v_portKey].IMPORT_YN == "1" && Number(JongmokImportDateList[v_portKey].startDate) <= Number(p_param.F12506)) {
        ...
        krwInfo.importance = (Number(krwInfo.importance) + (Number(v_portItem.importance) / numInfo.IMPORTANCE_FIX_NUM));
        /* 항목정보 설정 */
        v_portItem.F12506 = p_param.F12506;
        v_portItem.importance = 0; /* 비중 */
        v_portItem.F15007 = 0; /* 기준가 ( 전일 종가 ) - 기준가 */
      ...
      p_jongmok[v_portKey].importance = (Number(p_jongmok[v_portKey].importance) / numInfo.IMPORTANCE_FIX_NUM);
      /* 시가기준 시총 = 상장주식수(p_param.F16143) * 종가(p_param.F30700) */
      p_jongmok[v_portKey].F15028_1 = Number(fn_calc_data("F15028_1", {
      ...
      p_jongmok[v_portKey].F15028_2 = Number(fn_calc_data("F15028_2", {
    ...
    if(["1", "2"].includes(v_portItem.importance_method_cd)) {
      ...
      if(krwInfo.KRW_exists_yn == "Y") {
        ...
        /* 시가기준 시총 = 상장주식수(p_param.F16143) * 종가(p_param.F30700) */
        p_jongmok[krwInfo.F16013].F15028_1 = Number(fn_calc_data("F15028_1", {
          F30700: p_jongmok[krwInfo.F16013].F30700 /* 현재가 ( 당일 종가 ) - 종가 */ ,
          F16143: p_jongmok[krwInfo.F16013].F16143 /* 상장주식수 */
        }));
        /* 시가기준 시총 = 상장주식수(p_param.F16143) * 기준가(p_param.F15007) */
        p_jongmok[krwInfo.F16013].F15028_2 = Number(fn_calc_data("F15028_2", {
...
var fn_set_today_rate = function(
  ...
  try {
    ...
    for(var i = 0; i < Object.keys(p_jongmok).length; i++) {
      ...
      if(["1", "2"].includes(v_dataItem.importance_method_cd)) {
        switch (p_param.case_gubun) {
          case "case1":
            ...
            v_dataItem.TODAY_RATE = Number(fn_calc_data("TODAY_RATE1", {
          ...
          case "case3":
             ...
             */
              ...
              v_dataItem.TODAY_RATE = Number(fn_calc_data("TODAY_RATE2", {
                importance: v_dataItem.importance /* 비중 */ ,
      ...
      v_dataItem.F15028_J = Number(fn_calc_data("F15028_C", {
      ...
      if(p_param.first_record_yn == "Y") {
        /* 비교 시가총액 = 종가(p_param.F30700) * 상장주식수(p_param.F16143) * 지수적용비율(p_param.TODAY_RATE) */
        v_dataItem.F15028_C = Number(fn_calc_data("F15028_C", {
          F30700: v_dataItem.F30700 /* 현재가 ( 당일 종가 ) - 종가 */ ,
          F16143: v_dataItem.F16143 /* 상장주식수 */ ,
          TODAY_RATE: v_dataItem.TODAY_RATE /* 지수적용비율 */
        }, {}));
        ...
        /* 기준 시가총액 = 기준가(p_param.F15007) * 상장주식수(p_param.F16143) * 지수적용비율(p_param.TODAY_RATE) */
        v_dataItem.F15028_S = Number(fn_calc_data("F15028_S_2", {
      ...
      /* 당일 비교시총 총액 */
      p_daily.tot_F15028_J = Number(p_daily.tot_F15028_J) + Number(v_dataItem.F15028_J);
      /* 종가 * 상장주식수 * 시가총액 */
      v_dataItem.F15028_1_TODAY_RATE = Number(fn_calc_data("F15028_S_1", {
        F30700: v_dataItem.F30700 /* 종가 */ ,
        F16143: v_dataItem.F16143 /* 상장주식수 */ ,
        TODAY_RATE: v_dataItem.TODAY_RATE /* 지수적용비율 */
      }, {}));
      /* 기준가 * 상장주식수 * 시가총액 */
      v_dataItem.F15028_2_TODAY_RATE = Number(fn_calc_data("F15028_S_2", {
        F15007: v_dataItem.F15007 /* 기준가 ( 전일 종가 ) - 기준가 */ ,
        F16143: v_dataItem.F16143 /* 상장주식수 */ ,
        TODAY_RATE: v_dataItem.TODAY_RATE /* 지수적용비율 */
      }, {}));
      ...
      p_daily.tot_F15028_1_TODAY_RATE = (Number(p_daily.tot_F15028_1_TODAY_RATE) + Number(v_dataItem.F15028_1_TODAY_RATE));
      /* 기준가 * 상장주식수 * 시가총액 총합 */
      if(typeof p_daily.tot_F15028_2_TODAY_RATE == "undefined") {
        p_daily.tot_F15028_2_TODAY_RATE = 0;
      }
      p_daily.tot_F15028_2_TODAY_RATE = (Number(p_daily.tot_F15028_2_TODAY_RATE) + Number(v_dataItem.F15028_2_TODAY_RATE));
      /* ( 첫날 종가, 이후 기준가 ) * 상장주식수 * 시가총액 총합 */
      if(typeof p_daily.tot_F15028_S_TODAY_RATE == "undefined") {
        p_daily.tot_F15028_S_TODAY_RATE = 0;
      }
      p_daily.tot_F15028_S_TODAY_RATE = (Number(p_daily.tot_F15028_S_TODAY_RATE) + Number(v_dataItem.F15028_S));
      /* 기준가 * 상장주식수 총액 */
    ...
    for(var i = 0; i < Object.keys(p_jongmok).length; i++) {
      ...
      v_dataItem.AFTER_IMPORTANCE = Number(fn_calc_data("F15028_S_importance", {
      ...
      /* 기준 시가총액 비중 = 종가 시가총액( p_param.F15028_J ) / 시가 기준시총 총액 ( p_totalInfo.tot_F15028_S )  */
      v_dataItem.TODAY_IMPORTANCE = Number(fn_calc_data("F15028_S_importance", {
        F15028_S: v_dataItem.F15028_J /* 종가 * 상장주식수 * 지수적용비율 */
      }, {
        tot_F15028_S: p_daily.tot_F15028_J /* 종가 * 상장주식수 * 지수적용비율 총합 */
      }));
      if(typeof p_prev_jongmok[v_dataKey] != 'undefined') {
        /* 기준 시가총액 비중 = 기준 시가총액( p_param.F15028_S ) / 시가 기준시총 총액 ( p_totalInfo.tot_F15028_S )  */
        v_dataItem.BEFORE_IMPORTANCE = Number(fn_calc_data("F15028_S_importance", {
    ...
    if(p_daily.rebalancing != "1" && p_param.v_change_yn == "N") {
      ...
      p_daily.tot_F15028_S = Number(fn_calc_data("tot_F15028_S", {}, {
    ...
    p_daily.INDEX_RATE = Number(fn_calc_data("INDEX_RATE", {}, {
    ...
    p_daily.RETURN_VAL = Number(fn_calc_data("RETURN_VAL", {}, {
...
var fn_get_event_check = function(
  ...
  try {
    for(var i = 0; i < Object.keys(p_jongmok).length; i++) {
      ...
      v_dataItem.F15007_F16143 = Number(fn_calc_data("F15007_F16143", {
        F15007: v_dataItem.F15007 /* 기준가 ( 전일 종가 ) - 기준가 */ ,
        F16143: v_dataItem.F16143 /* 상장주식수 */
      }, {}));
      /* 기준 시가총액 = 기준가(p_param.F15007) * 상장주식수(p_param.F16143) * 지수적용비율(p_param.TODAY_RATE) */
      ...
      /* 당일 기준시총 총액 */
      eventObj.tot_F15028_S = Number(eventObj.tot_F15028_S) + Number(v_dataItem.F15028_S);
      /* 당일 비교시총 총액 */
      eventObj.tot_F15028_C = Number(eventObj.tot_F15028_C) + Number(v_dataItem.F15028_C);
      /* 기준가 * 상장주식수 총액 */
      eventObj.tot_F15007_F16143 = Number(eventObj.tot_F15007_F16143) + Number(v_dataItem.F15007_F16143);
    }
    /* 전일 비교시총 총액 */
    ...
    switch (p_gubun) {
      ...
      case "TODAY_RATE2":
        ...
        if(!p_param.F15028 || p_param.F15028 == 0) {
          ...
          v_calc = ((((Number(p_param.F30700) * Number(p_param.F16143) * Number(p_param.TODAY_RATE)) / Number(p_totalInfo.prev_tot_F15028_C)) * Number(p_totalInfo.tot_F15007_F16143)) / Number(p_param.F15007_F16143)).toFixed(17);
        }
        if(isNaN(v_calc)) v_calc = 0;
        break;
 ...
 * 2019-08-14  bkLove(촤병국)
 */
var fn_calc_jongmok = function(p_jongmok) {
  try {
    if(p_jongmok && Object.keys(p_jongmok).length > 0) {
      ...
      for(var i = 0; i < Object.keys(p_jongmok).length; i++) {
        ...
        /* 비교 시가총액 = 종가(p_param.F30700) * 상장주식수(p_param.F16143) * 지수적용비율(p_param.TODAY_RATE) */
        v_itemObj.F15028_C = Number(fn_calc_data("F15028_C", {
          F30700: v_dataItem.F30700 /* 현재가 ( 당일 종가 ) - 종가 */ ,
          F16143: v_dataItem.F16143 /* 상장주식수 */ ,
          TODAY_RATE: v_dataItem.TODAY_RATE /* 지수적용비율 */
        }, {}));
        /* 기준 시가총액 = 기준가(p_param.F15007) * 상장주식수(p_param.F16143) * 지수적용비율(p_param.TODAY_RATE) */
        v_itemObj.F15028_S = Number(fn_calc_data("F15028_S_2", {
          F15007: v_dataItem.F15007 /* 기준가 ( 전일 종가 ) - 기준가 */ ,
          F16143: v_dataItem.F16143 /* 상장주식수 */ ,
          TODAY_RATE: v_dataItem.TODAY_RATE /* 지수적용비율 */
        }, {}));
        /* 기준 시가총액 = 기준가(p_param.F15007) * 상장주식수(p_param.F16143) * 지수적용비율(p_param.TODAY_RATE) */
        v_itemObj.F15007_F16143 = Number(fn_calc_data("F15007_F16143", {
          F15007: v_dataItem.F15007 /* 기준가 ( 전일 종가 ) - 기준가 */ ,
          F16143: v_dataItem.F16143 /* 상장주식수 */
        }, {}));
        /* 기준 시가총액 = 기준가(p_param.F15007) * 상장주식수(p_param.F16143) * 지수적용비율(p_param.TODAY_RATE) */
        v_itemObj.F30700_F16143 = Number(fn_calc_data("F30700_F16143", {
